    ...
    def forward(
        ...
        for prefill_cache_len, prefill_block_table, query_len in zip(
            ...
            with self.get_timer(OperationMetrics.ATTN_PREFILL, layer_id):
                # # 计算实际需要attend的cache长度
                # if use_sliding_window:
                #     # 滑动窗口：最多只attend sliding_window个token
                #     effective_cache_len = min(
                #         prefill_cache_len.item() + query_len, 
                #         sliding_window
                #     )
                #     cache_seqlens = torch.tensor(
                #         [effective_cache_len], dtype=torch.int32, device=self.device
                #     )
                # else:
        ...
        with self.get_timer(OperationMetrics.ATTN_DECODE, layer_id):
            # # 对于滑动窗口，限制cache_seqlens
            # if use_sliding_window:
            #     effective_decode_cache_len = torch.clamp(
            #         self.decode_cache_len, max=sliding_window
            #     )
            # else:
            #     effective_decode_cache_len = self.decode_cache_len
