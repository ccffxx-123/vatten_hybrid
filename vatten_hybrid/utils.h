...
// physical memory pages for vmm backend (default)
std::vector<CUmemGenericAllocationHandle> cuda_pages;

// (reqID, offset, layer_idx) -> (k_page, v_page)，用于后续unmap查找物理页
...
// utils.h - Add new type definitions

enum class LayerType {
    TRANSFORMER = 0,  // Standard full attention
    SWA = 1,          // Sliding Window Attention
    MAMBA = 2,        // Mamba/Linear Attention (static state)
    INVALID = -1
};
...
struct LayerConfig {
    LayerType type;
    int global_layer_idx;      // Position in the full model
    int internal_layer_idx;    // Index within same-type layers
    
    // Type-specific parameters
    union {
        struct {
            // Transformer: no special params (uses global max_context_length)
        } transformer;
        struct {
            u64 window_size;   // Number of tokens in sliding window
        } swa;
        struct {
            u64 state_size;    // Fixed state dimension (d_state * d_model)
        } mamba;
    } params;
};
...
struct ModelTopology {
    ...
    int num_swa_layers;
    int num_mamba_layers;
    
    // Derived mappings (built during init)
    std::map<int, std::pair<LayerType, int>> global_to_internal;
};

// Per-type tensor metadata (没用到？)
struct TensorTypeMetadata {
    LayerType type;
    int num_layers;                    // Number of layers of this type
    std::vector<at::Tensor> k_tensors;
    std::vector<at::Tensor> v_tensors;
    u64 virt_buff_size;
    u64 virt_buff_size_per_req;
    
    // Type-specific allocation state
    std::vector<u64> mapped_pages;     // Per-request page count
    std::vector<u64> curr_seq_lengths; // Per-request sequence length
    
    // For SWA: circular buffer state
    std::vector<u64> ring_head_offset; // Current head position in ring
};