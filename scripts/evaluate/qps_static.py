...
# 2. 编写正则表达式，从文件夹名称中提取 backend 和 cl
# 针对样例: model_gemma-3-4b_tp_1_attn_fi_paged_16_cl_32768_pd_500_reqs_10_bs2
# 匹配 "attn_" 后面到 "_cl_" 之间的内容作为 backend，匹配 "_cl_" 后面的数字作为 CL
dir_pattern = re.compile(r"attn_(.+?)_cl_(\d+)")
...
for folder_name in os.listdir(base_dir):
    ...
    cl = int(match.group(2))   # 例如: 32768 (Context Length)
...
# 按 Backend 和 CL 排序，保证画图时折线点是从左到右按 CL 递增的
summary_df = summary_df.sort_values(by=['Backend', 'CL'])


# ================== 新增：在终端输出数据 ==================
print("\n" + "="*60)
print("📊 实验数据汇总结果 (按 Backend 和 CL 排序):")
print("="*60)
...
for i, backend in enumerate(backends):
    ...
    # (a) E2EL 曲线
    axes[0].plot(sub_df['CL'], sub_df['E2EL'], marker=markers[i % len(markers)], 
                 linewidth=2, markersize=6, label=backend)
    
    # (b) TTFT 曲线
    axes[1].plot(sub_df['CL'], sub_df['TTFT'], marker=markers[i % len(markers)], 
                 linewidth=2, markersize=6, label=backend)
    
    # (c) TPOT 曲线
    axes[2].plot(sub_df['CL'], sub_df['TPOT'], marker=markers[i % len(markers)], 
                 linewidth=2, markersize=6, label=backend)
...
for i in range(3):
    axes[i].set_xlabel('Context Length (CL)', fontsize=12) # <--- 修改了这里的 X 轴标签
...
# 自动调整间距并保存
plt.tight_layout()
plt.savefig('attention_backends_comparison_static.png', bbox_inches='tight', dpi=300)
print("绘制成功！图表已保存为当前目录下的 attention_backends_comparison_static.png")
