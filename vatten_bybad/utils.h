...
std::vector<u64> mapped_pages_trans, mapped_pages_swa, mapped_pages_state; // 每个请求当前已分配(映射)的物理页数量（单位1表示一个token所有层的K和V）
...
{
    // if(is_only) num_tokens = std::min(num_tokens, windows_size);
