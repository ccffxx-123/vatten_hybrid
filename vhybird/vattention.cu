...
public:
    ...
    void init_buffer_sizes()
    {
        ...
        //state
        conv_elems_per_slot = (u64)conv_dim * conv_kernel_m1 * num_layers_state;
        ssm_elems_per_slot = (u64)mamba_num_heads * mamba_head_dim * d_state * num_layers_state;

        exact_state_size_per_req = (conv_elems_per_slot + ssm_elems_per_slot) * bytes_per_elem;
                      ...
                      py::object dtype_)
    {
        ...
        for(int i = 0; i < mamba_params_.size(); i++) {
            assert(mamba_params_[i] >= 0 && mamba_params_[i] < 1000000);
        }
        conv_dim = mamba_params_[0];
        conv_kernel_m1 = mamba_params_[1];
        mamba_num_heads = mamba_params_[2];
        mamba_head_dim = mamba_params_[3];
        d_state = mamba_params_[4];
    ...
    at::Tensor alloc_virtual_tensor(int num_layers, bool if_state)
    {
        ...
        if(!if_state)
            ...
            // flat 二维：[batch, 总元素数]
            long total_elems = (long)(conv_elems_per_slot + ssm_elems_per_slot);
            shape = {max_batch_size, total_elems};
        }
