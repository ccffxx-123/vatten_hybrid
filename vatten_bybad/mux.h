...
// State (typeId == 2): Mamba state，K/V 共享同一物理页
inline void map_pages_state(int reqId, u64 req_offset, CUdeviceptr state_ptr)
{
    CUPage page = pop_cuda_page();
    // K/V 使用同一物理页（state 不区分 K/V）
    map_cuda_pages(reqId, /*typeId=*/2, req_offset, state_ptr, state_ptr, page, page);
}
...
inline void unmap_pages_state(int reqId, u64 req_offset, CUdeviceptr state_ptr)
{
    ...
    push_cuda_page(pages.first); // state 只有一个物理页 (first == second)
