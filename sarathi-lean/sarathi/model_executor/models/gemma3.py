...
def _make_rope_scaling(config_rope_scaling: Optional[Dict]) -> Optional[Dict]:
    ...
    HF Gemma3 uses ``rope_type`` as the key; get_rope() expects ``type``.
    """
...
class Gemma3Attention(nn.Module):
    ...
    def __init__(
        ...
        if attention_type == "sliding_attention":
            ...
            # Global layers: large base + linear scaling
            self.rotary_emb = get_rope(
                head_size=self.head_dim,
                rotary_dim=self.head_dim,
                max_position=max_position_embeddings,
...
# gemini
#  # coding=utf-8
# """Inference-only Gemma 3 model compatible with HuggingFace weights.

# Gemma 3 uses interleaved sliding window and full attention layers.
# This implementation strips out the vision encoder and supports purely the text LLM core.
# """
# from typing import Any, Dict, List, Optional
...
#     """Gemma models typically use GeGLU (GELU gated linear unit) with tanh approximation."""
#     def forward(self, x: torch.Tensor) -> torch.Tensor:
...
#                 "Only gelu_pytorch_tanh is supported for Gemma 3."
#             )
...
#         # Gemma uses query_pre_attn_scalar if present, otherwise standard scaling
#         scalar = getattr(config, "query_pre_attn_scalar", self.head_dim)
#         self.scaling = scalar ** -0.5
        
#         self.layer_id = layer_id

#         # Mixed Attention Routing parameters
...
#         # Dynamic pass for Gemma 3's mixed attention
#         attn_output = get_attention_wrapper().forward(
#             q,
#             k,
#             v,
...
#         self.self_attn = Gemma3Attention(config=config, layer_id=layer_id)
#         self.mlp = Gemma3MLP(
...
#         self.layers = nn.ModuleList(
#             [
#                 Gemma3DecoderLayer(config, layer_id=layer_id + layer_offset)
...
#     def __init__(self, config: Any) -> None:
#         super().__init__()
#         # Ensure we are purely using the inner text_config for the core LLM execution
#         self.config = config
#         self.text_config = getattr(config, "text_config", config)
        
#         self.model = Gemma3Model(self.text_config)
#         vocab_size = ((self.text_config.vocab_size + 63) // 64) * 64
...
#         if not self.is_pipeline_first_stage:
#             hidden_states = torch.empty(
#                 (positions.shape[0], self.text_config.hidden_size),
#                 dtype=self.text_config.dtype,
#                 device=hidden_states.device,
#             )
#             hidden_states = recv(hidden_states)
...
#         """Map generic/multimodal HF weights to pure text execution format."""
#         # 1. Strip the multimodal vision prefixes
#         if name.startswith("language_model."):
#             name = name.replace("language_model.", "")
#         elif name.startswith("text_model."):
#             name = name.replace("text_model.", "")
            
#         # 2. Map standard LLaMA-style naming (Gemma uses the exact same QKV / GateUp structures)
#         replacements = [
...
#         assert self.text_config.num_hidden_layers % pp_size == 0
#         layers_per_stage = self.text_config.num_hidden_layers // pp_size
...
#         q_proj_shard_size = (self.text_config.num_attention_heads // tp_size) * head_dim
#         kv_proj_shard_size = (self.text_config.num_key_value_heads // tp_size) * head_dim
...
#         for original_name, loaded_weight in hf_model_weights_iterator(
#             model_name_or_path, cache_dir, load_format, revision
#         ):
#             # Ignore vision encoder completely
#             if "vision" in original_name or "multi_modal_projector" in original_name:
#                 continue
                
#             name = self._map_weight_name(original_name)
...
#             # Layer indexing adjustment for pipeline parallelism
...
#             # QKV projection weights mapping 
#             is_attention_weight = False
#             for weight_name, shard_size, offset in attention_weight_specs:
#                 if f"self_attn.{weight_name}" not in name:
#                     continue
...
#             # Gate/Up projection weights mapping
#             is_gate_up_weight = False
#             for stride_id, weight_name in enumerate(["gate_proj", "up_proj"]):
#                 if f"mlp.{weight_name}" not in name:
#                     continue
